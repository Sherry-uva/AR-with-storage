...
lossRate = 1e-11
num100GIP = float(sys.argv[5])
runIndex = int(sys.argv[6])


output = 'AR-K' + str(numCustomers) + '-N' + str(numHighSpeedPorts) + '-W' + str(int(W)) + '-load' + str(int(load))  + '-lossRate' + str(lossRate) + '-num100GIP' + str(int(num100GIP)) + '-runIndex' + str(runIndex) + '.csv'
...
# 	while delayedTransfers < G.numDelayed and totalTransfers < 10**7:
	while totalTransfers < G.numTotalTransfers:
		msg = yield pipe.get()
		if msg[0] == 'newRequest':
		...
		if msg[0] == 'transferFinish':
			...
			if msg[1] == True:
				...
				if totalTransfers%10000 == 0:
					print str(delayedTransfers) + ', ' + str(totalTransfers)
#					ipPathProc.interrupt(['simFinish', totalTransfers])
...
# 	if delayedTransfers < G.numDelayed:
	...
	env.timeout(1)
 	...
 	postProcessing.parser('AR', numCustomers, numHighSpeedPorts, W, load, lossRate, num100GIP, runIndex)
